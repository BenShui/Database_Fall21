...
begin
	...
	begin
	    ...
	    insert into #array(step, cy_res,num) values(@step, @str, @test_1) --将当前成语插入临时表
	declare cur  scroll cursor for                       --定义游标，寻找成语表中开头字等于当前结尾字的成语
	    select cy from idioms where substring(cy,patindex('%[吖-咗]%',cy),1)=@ch
	open cur
	...
	begin
	    ...
	    select @test_1 = count(*) from idioms
		    where substring(cy,patindex('%[吖-咗]%',cy),1)=@ch
